...
class contact extends CI_Controller {
	...
	{
		// Manatory
		$data['about'] = $this->mdl_news->get_company_profile('about');
		$data['about_short'] = $this->mdl_news->get_company_profile('about_short');
		$contact= $this->mdl_news->get_company_profile('kontak');
		$data['contact'] = $contact->value;
		$data['social_media'] = $this->mdl_news->get_social_media();

		//perpages
		$data['recent_news'] = $this->mdl_news->get_recent_news();
		// CONTACT 
		$this->load->library('googlemaps');
		// SET MAP
		$config['center'] = '-6.2, 106.816667';
		$config['zoom'] = '12';
		$this->googlemaps->initialize($config);
		$marker = array();
		$marker['position'] = '-6.195618, 106.821039';
		$marker['draggable'] = FALSE;
		$marker['animation'] = 'DROP';
		$marker['onclick'] = 'Bambuwulung';
		$this->googlemaps->add_marker($marker);
		$data['map'] = $this->googlemaps->create_map();

		$data['toggle_map'] = "btn btn-primary";
		$data['toggle_list'] = "btn btn-default";

		$this->menu_handler->set_menu_frontend('contact','Partner',$data,2,3);
	}


	function news_detail($id_news){
		$data['about'] = $this->mdl_news->get_company_profile('about');
		$data['about_short'] = $this->mdl_news->get_company_profile('about_short');
		$contact= $this->mdl_news->get_company_profile('kontak');
		$data['contact'] = $contact->value;
		...
		$data['news'] = $this->mdl_news->get_news_byid($id_news);
		
		$data['news_lastest'] = $this->mdl_news->get_news_lastest($id_news);
		$data['id_news'] = $id_news;

		// print_r($data['news_lastest']);die();
		$this->menu_handler->set_menu_frontend('news','Partner',$data,2,3);
