...
<section class="content-header">
    <h1>
    <?php echo $title; ?>
    <small><?php if(isset($new_data)) { echo '<i class="fa fa-chevron-right"></i>&nbsp;&nbsp;Add'; } else { echo '<i class="fa fa-edit"></i>&nbsp;&nbsp;Edit'; } ?></small>
    </h1>
    <ol class="breadcrumb">
        <li><a href="<?php echo site_url('settings/settings_crud/0-0')?>" tooltip="Goto Dashboard"><i class="fa fa-dashboard"></i> Dashboard</a></li>
        <li><a href="<?php echo site_url('sliders/sliders_crud/0-0')?>"><?php echo $title; ?></a></li>
        <li class="active"><?php if(isset($new_data)) { echo '<i class="fa fa-plus"></i>&nbsp;&nbsp;Add'; } else { echo '<i class="fa fa-edit"></i>&nbsp;&nbsp;Edit'; } ?></li>
    </ol>
</section>
        ...
        <div class="col-xs-12">
            ...
            <div class="box box-primary">	
                <div class="box-header with-border">
                  <h3 class="box-title">Detail Sliders</h3>
                </div><!-- /.box-header -->
                <div class="box-body">
	                <?php	if(@$status){ ?>
					...
					<?php } ?>
                 	...
                 	<?php
						if(isset($new_data)) {
							echo form_open_multipart('sliders/add_slider','class="form-horizontal" role="form"');
							$hidden_data_user = array(
								'new_data'=>true
							);
							echo form_hidden($hidden_data_user);
						} else {
							echo form_open_multipart('sliders/edit_slider','class="form-horizontal" role="form"');
							$hidden_data_user = array(
								'id' => $data->id,
								'new_data'=> false
					...
					<div class="form-group">
						<label class="col-sm-2 control-label">Start Publish</label>
						<div class="col-sm-5">
							<input type="text" class="form-control" autocomplete="off" placeholder="Start date" id="dateStartPicker" name="start_publish" <?php if(!isset($new_data)){ 
								$fStartDate = date("m/d/Y", strtotime($data->start_publish));
					...
					<div class="form-group">
						<label class="col-sm-2 control-label">End Publish</label>
						<div class="col-sm-5">
							<input type="text" class="form-control" autocomplete="off" placeholder="End date"  id="dateEndPicker" name="end_publish" <?php if(!isset($new_data)){ 
								$fEndDate = date("m/d/Y", strtotime($data->end_publish));
								echo 'value="'.$fEndDate.'"'; 
								} ?> >
						</div>
					</div>
